...
class App {
    constructor(font) {
        ...
        document.getElementById('inputButton').addEventListener('click', this.calculate.bind(this), false)
    ...
    updateGlyph() {
        ...
        this.directPoints = pointData['direct'];
        this.indirectPoints = pointData['indirect'];

        var glyphData = this.replaceGlyph();
        this.glpyhWidth = glyphData['width'];
        this.glyphHeight = glyphData['height'];
    }

    calculate() {
        var glyphArea = Math.abs(polygonArea(this.directPoints));
        var boxArea = this.glpyhWidth * this.glyphHeight;
        var ratio = glyphArea / boxArea * 100;
        // console.log(this.str, glyphArea, boxArea, glyphArea/boxArea);
        console.log(this.str, ratio.toFixed(2));
    }
...
window.onload = () => {
    ...
    var fontName = 'TmonMonsori.otf.otf';
